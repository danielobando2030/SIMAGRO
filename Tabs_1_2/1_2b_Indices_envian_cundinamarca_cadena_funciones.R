...
#Fecha de ultima modificacion: 11/10/2025
#Modificas: Cristian Daniel Obando Arbeláez,
################################################################################-
...
cundinamarca<-readRDS("base_indices_cundinamarca_cadena_1_2.RDS")
cundinamarca$depto_origen[cundinamarca$depto_origen=="QUINDÍO"]<-"QUINDIO"

cun_en_col<-function(Año = NULL, Mes = NULL, Producto = NULL){
  ...
  p <- leaflet(mapa) %>%
    ...
    addPolygons(fillColor = ~my_palette(columna_porcentaje),
                ...
                popup = ~paste0("<strong>Departamento: </strong>", dpto_cnmbr, 
                                "<br><strong>Porcentaje enviado a cundinamarca:</strong>", round(columna_porcentaje, digits = 1),"%",
                                "<br><strong>Toneladas: </strong>", formatC((total_ton), format = "f", big.mark = ".",decimal.mark = ",", digits = 1), 
                                "<br><strong>Porcentaje que representa para cundinamarca:</strong>", round(represent_ant*100, digits = 1),"%"),
  ...
  df_sin_cundinamarca <- df[!df$depto_origen%in%c("BOGOTÁ, D.C.","CUNDINAMARCA"),]
  # Calcula el porcentaje máximo y el departamento correspondiente
  porcentaje_max <- round(max(df_sin_cundinamarca$columna_porcentaje) * 100)
  dpto_max <- df_sin_cundinamarca$depto_origen[which.max(df_sin_cundinamarca$columna_porcentaje)]
  porcentaje_max_1 <- round(max(df$columna_porcentaje[which(df$depto_origen%in%c("BOGOTÁ, D.C.","CUNDINAMARCA"))]) * 100) 
