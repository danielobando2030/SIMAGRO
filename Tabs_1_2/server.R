  ...
  if (is.data.frame(res) || is.list(res)) {
    if(nrow(res$datos) == 0) {
      ...
      if (input$producto == "todo") {
        values$subtitulo <- (paste0("Para el producto y periodo de tiempo  seleccionado, el ", porcentaje_max ,"%. del volumen de alimentos reportada en ",dpto_max, " van hacia Cundinamarca"))
      } else if (input$producto != "todo") {
        values$subtitulo <- (paste0("Para el producto y periodo de tiempo  seleccionado, el ", porcentaje_max ,"%. del volumen de ",input$producto," reportada en ",dpto_max, " van hacia Cundinamarca"))
      }
    }
  }
