...
library(glue);library(tidyverse);library(gridExtra);library(corrplot);library(treemapify);library(highcharter);library(arrow)
options(scipen = 999)
################################################################################-

salen<-readRDS("base_porcentaje_productos_salen_1_6.RDS")
...
salen_prod<-function(año = NULL, Mes = NULL, depto = NULL){
  ...
  titulo <- paste0("Productos que salen de Cundinamarca", ifelse(is.null(año),"",paste0(" para ",año)),
