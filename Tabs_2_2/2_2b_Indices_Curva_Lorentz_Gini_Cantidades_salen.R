...
abastecimiento_resto_ANO_mes_alimento=readRDS("abastecimiento_resto_ANO_mes_alimento_2_2.RDS")
abastecimiento_resto_ANO_alimento=readRDS("abastecimiento_resto_ANO_alimento_2_2.RDS")
abastecimiento_resto_alimento=readRDS("abastecimiento_resto_alimento_2_2.RDS")
abastecimiento_resto_total=readRDS("abastecimiento_resto_total_2_2.RDS")
abastecimiento_resto_ANO=readRDS("abastecimiento_resto_ANO_2_2.RDS")
abastecimiento_resto_ANO_mes=readRDS("abastecimiento_resto_ANO_mes_2_2.RDS")
...
Lorentz_GINI = function(ANO=NULL, ALIMENTO=NULL, MES=NULL){
  if (is.null(ANO) && is.null(ALIMENTO) && is.null(MES)) {
    df=abastecimiento_resto_total
  }
  
  # 2. Filtro solo por alimento
  if (is.null(ANO) && !is.null(ALIMENTO) && is.null(MES)) {
    df=subset(abastecimiento_resto_alimento, producto == ALIMENTO)
  }
  
  # 3. Filtro por año y alimento
  if (!is.null(ANO) && !is.null(ALIMENTO) && is.null(MES)) {
    df=subset(abastecimiento_resto_ANO_alimento,
                  anio == ANO & producto == ALIMENTO)
  }
  
  # 4. Filtro por año, alimento y mes
  if (!is.null(ANO) && !is.null(ALIMENTO) && !is.null(MES)) {
    df=subset(abastecimiento_resto_ANO_mes_alimento,
                  anio == ANO & producto == ALIMENTO & mes == MES)
  }
  
  # 5. Filtro solo por año
  if (!is.null(ANO) && is.null(ALIMENTO) && is.null(MES)) {
    df=subset(abastecimiento_resto_ANO, anio == ANO)
  }
  
  # 6. Filtro por año y mes (sin alimento)
  if (!is.null(ANO) && is.null(ALIMENTO) && !is.null(MES)) {
    df=subset(abastecimiento_resto_ANO_mes,
                  anio == ANO & mes == MES)
  }
  
  # 7. Filtro solo por mes (sin año ni alimento)
  if (is.null(ANO) && is.null(ALIMENTO) && !is.null(MES)) {
    df=subset(abastecimiento_resto_mes, mes == MES)
  }
  ...
  #  geom_point(aes(x=coord_x, y=coord_y), color="darkorange", size=4) +
    ...
    labs(x="Proporción acumulada de Municipios de destino",
         y="Proporción acumulada volumen") +
