...
grafica_indice_mun <- function(tipo, anio_seleccionado = "", productos_seleccionados = "") {
  ...
  if (tipo == 2) {
    ...
    df$tooltip_text <- paste("Año: ", df$fecha , "<br> Producto:",df$producto, "<br> IHH:" , format(round(df$IHH,1),decimal.mark=",",big.mark="."))
   ...
   df$mes_nombre <- nombres_meses[df$month]
    ...
    df <- df[df$producto %in% productos_seleccionados, ]
    df$tooltip_text <- paste("Año: ", df$year ,"<br> Mes:",df$mes_nombre, "<br> Producto:",df$producto, "<br> IHH:" , format(round(df$IHH,1),decimal.mark=",",big.mark="."))
    p_plano <- ggplot(df, aes(x = fecha, y = IHH, color = producto)) +
      geom_line() +
      geom_point(aes(text = tooltip_text),size = 1e-8) +
      labs(x = "Fecha", y = "Índice diversidad de origen") +
    ...
    if ( anio_seleccionado != "") {
      ...
      #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
       ...
       df <- rename(df, fecha = year) 
       df$tooltip_text <- paste("Año: ", df$fecha , "<br> IHH:" , format(round(df$IHH,1),decimal.mark=",",big.mark="."))
      ...
      df$mes_nombre <- nombres_meses[df$month]
      df <- rename(df, fecha = mes_y_ano)
      df$tooltip_text <- paste("Año:", df$year ,"<br> Mes:",df$mes_nombre, "<br> IHH:" , format(round(df$IHH,1),decimal.mark=",",big.mark="."))
      p_plano <- ggplot(df, aes(x = fecha, y = IHH)) +
        geom_line(color = "#983136") +
        geom_point(aes(text = tooltip_text),size = 1e-8) +
        labs(x = "Fecha", y = "Índice diversidad de origen") +
