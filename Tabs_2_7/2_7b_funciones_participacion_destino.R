  ...
  GINI_anual_total <- readRDS("base_indice_anual_total_destino_2_7.RDS")
  GINI_anual_total$year <-as.numeric(GINI_anual_total$year)
  GINI_anual_producto <- readRDS("base_indice_anual_producto_desti_2_7.RDS")%>%
    mutate(producto=tools::toTitleCase(tolower(producto)))
  GINI_anual_producto$year <- as.numeric(GINI_anual_producto$year)
  GINI_mensual_producto <- readRDS("base_indice_mensual_producto_destino_2_7.RDS")%>%
    mutate(producto=tools::toTitleCase(tolower(producto)))
  GINI_mensual_total <- readRDS("base_indice_mensual_total_destino_2_7.RDS")
...
grafica_indice <- function(tipo, anio_seleccionado = "", productos_seleccionados = "") {
  if (tipo == 1 ) {
    ...
    df <- GINI_anual_producto
    df <- df %>%
      select("year","producto", "GINI")
    
  } else if (tipo == 3) {
    df <- GINI_mensual_total
    df <- df %>%
      select("mes_y_ano","year","month","GINI")
    if (anio_seleccionado != ""){
      df <- df %>%
        filter(anio_seleccionado == year)
    }
  } else if (tipo == 4) {
    df <- GINI_mensual_producto
    df <- df %>%
      select("year","month","mes_y_ano","producto", "GINI")
    if (anio_seleccionado != ""){
      df <- df %>%
        filter(anio_seleccionado == year)
    }
  ...
  df<-df%>%mutate(GINI=round(GINI*100))
  ...
  if (tipo %in% c(1)) {
    ...
    df$tooltip_text <- paste("Año: ", df$fecha , "<br> GINI:" , format(round(df$GINI,1),decimal.mark=",",big.mark="."))  
    ...
    df$tooltip_text <- paste("Año: ", df$fecha , "<br> Producto:",df$producto, "<br> GINI:" , format(round(df$GINI,1),decimal.mark=",",big.mark="."))
    p_plano<-ggplot(df, aes(x = fecha, y = GINI, color = producto)) +
      geom_line() +
      geom_point(aes(text = tooltip_text), size = 1e-8) +
      scale_color_manual(values = col_palette) +
      labs(x = "Fecha", y = " ") +
    ...
    df$tooltip_text <- paste("Año:", df$year ,"<br> Mes:",df$mes_nombre, "<br> GINI:" , format(round(df$GINI,1),decimal.mark=",",big.mark="."))
   p_plano<- ggplot(df, aes(x = fecha, y = GINI)) +
      geom_line(color = "#983136") +
     geom_point(aes(text = tooltip_text), size = 1e-8) +
      labs(x = "Fecha", y = " ") +
   ...
   if ( anio_seleccionado != "") {
    ...
    df$mes_nombre <- nombres_meses[df$month]
    df <- df[df$producto %in% productos_seleccionados, ]
    df$tooltip_text <- paste("Año: ", df$year ,"<br> Mes:",df$mes_nombre, "<br> Producto:",df$producto, "<br> GINI:" , format(round(df$GINI,1),decimal.mark=",",big.mark="."))
    p_plano<-ggplot(df, aes(x = fecha, y = GINI, color = producto)) +
      geom_line() +
      geom_point(aes(text = tooltip_text), size = 1e-8) +
      labs(x = "Año", y = " ") +
  ...
  indice_min_vulnerabilidad <- which.min(df$GINI)
  min_vulnerabilidad <- round(df$GINI[indice_min_vulnerabilidad], 3)
  fecha_min_vulnerabilidad <- df$fecha[indice_min_vulnerabilidad]
  producto_min_vulnerabilidad <- ifelse("producto" %in% names(df), df$producto[indice_min_vulnerabilidad], NA)
  ...
  return(list(
    ...
    datos = df,
    min_vulnerabilidad = min_vulnerabilidad,
    fecha_min_vulnerabilidad = fecha_min_vulnerabilidad,
    producto_min_vulnerabilidad = producto_min_vulnerabilidad
  ))
} 
