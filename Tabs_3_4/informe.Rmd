...
if (!is.null(params$resumen)) {
  writeLines(params$resumen)
} else {
  writeLines("No hay resumen disponible.")
}
```
  ...
  if (is.null(params$datos) || nrow(params$datos) == 0) {
    ...
    if (!all(c("ciudad", "comp") %in% names(df))) {
      ...
      df %>%
        ...
        dplyr::mutate(
          `Diferencia con Bogotá ($)` = formatC(`Diferencia con Bogotá ($)`,
                                                big.mark = ",", digits = 0, format = "f")
        ) %>%
        head(8) %>%
        kableExtra::kable(
          format = "latex",
