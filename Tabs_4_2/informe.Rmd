...
```{r mensajes_interpretativos, results='asis', echo=FALSE}
print_msg <- function(txt) {
  if (is.null(txt) || is.na(txt) || !nzchar(txt)) return()
  cat("\\begingroup\n")
  cat("\\fontsize{10}{10}\\selectfont\n")
  cat("\\textcolor{grayA}{", txt, "}\n", sep = "")
  cat("\\endgroup\n\n")
}

print_msg(params$mensaje3)
print_msg(params$mensaje2)
print_msg(params$mensaje1)
```
